const paginatedFetchJourney = (model) => {
  return async (req, res) => {
    ...
    if (depName) {
      query['Departure station name'] = depName;
    }
    if (retName) {
      query['Return station name'] = retName;
    }
    ...
    try {
      ...
      if (journeys.length === 0) {
        return res.status(404).json({
          message: 'There was no journey for these stations.',
          data: [],
        });
      }
      ...
      res.status(500).json({ message: 'Journeys fetch fail' });
    }
  };
};

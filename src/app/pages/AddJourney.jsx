...
const AddJourney = () => {
  ...
  const calculateRoute = useCallback(
    async (departure, destination) => {
      const response = await instance.post(`/api/journey-calc`, {
      ...
      const newData = data?.legs?.reduce(
        (acc, cur) => {
      ...
      setFormSubmit((prev) => ({
        ...prev,
        'Duration (sec)': newData?.totalTime,
        'Covered distance (m)': parseInt(newData?.totalDistance.toFixed(2)),
        Return: formatted,
      }));
    },
    [now]
  ...
  const onJourneyCreate = async (e) => {
    try {
      e.preventDefault();
      setLoading(true);
      const resp = await instance.post('/api/journey/add', {
        data: formSubmit,
      });
      setAlert({
        isOpen: true,
      ...
      <div className='map-container'>
        <MapContainer center={center} zoom={11} scrollWheelZoom={true}>
          ...
          <Marker
            icon={greenIcon}
            position={departure}
            eventHandlers={departureHandler}
            ref={departureMarkerRef}
          >
            <Popup>

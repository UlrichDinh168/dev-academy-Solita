  ...
  journeyFiles.forEach((file) => {
    ...
    const csvStream = fastcsv
      ...
      .on('end', async (rowCount) => {
        ...
        console.log(
          `Finished processing ${color.green(rowCount)} rows from ${color.cyan(
            file
          )} with ${color.redBright(remove)} invalid rows.\n`
        );

        if (filesProcessed === journeyFiles.length) {
          console.log(
            `Totally added ${color.green(
              totalRowCount
            )} documents to mongo and removed ${color.redBright(totalRemoveCount)} invalid rows.\n`
          );
          database.once('close', () => {
